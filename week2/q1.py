...
class History:
    ...
    def update_history(self, action):
        ...
        copied_history.append(action)
        return History(copied_history)
...
def backward_induction(history_obj):
    ...
    for square in history_obj.get_valid_actions():
        next_history_obj = history_obj.update_history(square)
    ...
    history_obj.update_history(best_square)
    history_str = ''.join(str(x) for x in history_obj.history)
    if history_obj.player == 'x':
        strategy_dict_x[history_str] = {str(i): 0 for i in range(9)}
        strategy_dict_x[history_str][str(best_square)] = 1
    else:
        strategy_dict_o[history_str] = {str(i): 0 for i in range(9)}
        strategy_dict_o[history_str][str(best_square)] = 1
